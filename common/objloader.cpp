...
bool loadASCII(
	...
	while( 1 ){
		...
		if ( strcmp( lineHeader, "Vertex" ) == 0 ){
			glm::vec3 vertex;
			glm::vec3 normal;
			fscanf(file, "%d %f %f %f {normal=(%f %f %f)}\n", &temp_order, &vertex.x, &vertex.y, &vertex.z, &normal.x, &normal.y, &normal.z );
			temp_vertices.push_back(vertex);
			temp_normals.push_back(normal);
		}else if ( strcmp( lineHeader, "Face" ) == 0 ){
			std::string vertex1, vertex2, vertex3;
			unsigned int vertexIndex[3], normalIndex[3];
			int matches = fscanf(file, "%d %d %d %d\n", &temp_order, &vertexIndex[0], &vertexIndex[1], &vertexIndex[2]);
			vertexIndices.push_back(vertexIndex[0]);
			vertexIndices.push_back(vertexIndex[1]);
			vertexIndices.push_back(vertexIndex[2]);
